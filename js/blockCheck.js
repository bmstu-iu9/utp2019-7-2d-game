const blockCheck = (o,i) => {
  if (o.id != 0) {
    ...
    if (InBlock(o.x, o.y, 'W')) {
      ...
      ctx.drawImage(water , ((((~~envN[1])) * 32) % 96), 0, 32, 32, NN[0] * (blocks[(~~((o.y + 48)  / 32)) * curLevel.map[0].length + (~~((o.x + 16)  / 32))].x  +  FF[0]),NN[1] * (blocks[(~~((o.y + 48)  / 32)) * curLevel.map[0].length + (~~((o.x + 16)  / 32))].y + FF[1]) ,NN[0] * 32, NN[1] * 32 );
      if (InBlock(o.x + 32, o.y, 'W')) {
        ctx.drawImage(water, ((((~~envN[1])) * 32) % 96), 0, 32, 32, NN[0] * (blocks[(~~((o.y + 48)  / 32)) * curLevel.map[0].length + (~~((o.x + 48)  / 32))].x  +  FF[0]),NN[1] * (blocks[(~~((o.y + 48)  / 32)) * curLevel.map[0].length + (~~((o.x + 48)  / 32))].y + FF[1]) ,NN[0] * 32, NN[1] * 32 );
      }
      if (InBlock(o.x - 32, o.y, 'W')) {
        ctx.drawImage(water, ((((~~envN[1])) * 32) % 96), 0, 32, 32, NN[0] * (blocks[(~~((o.y + 48)  / 32)) * curLevel.map[0].length + (~~((o.x - 16)  / 32))].x  +  FF[0]),NN[1] * (blocks[(~~((o.y + 48)  / 32)) * curLevel.map[0].length + (~~((o.x - 16)  / 32))].y + FF[1]) ,NN[0] * 32, NN[1] * 32 );
      }
    ...
    if (InBlock(o.x, o.y,'L')) {  //проверка на Фейк-блок и его последующее уничтожениеж
      o.hp = 0;
      deathlist.push(o);
      characters.splice(i,1);
    }
    ...
    if (InBlock(o.x, o.y, 'C')) {  //проверка на Фейк-блок и его последующее уничтожениеж
      BlockChange((o.x + 16) ,(o.y + 16),'C' ,' '); //смена по коду   0 - до , 1 - после
      BlockChange((o.x + 16) ,(o.y + 48),'C' ,' '); //смена по коду   0 - до , 1 - после
...
const BlockChange = (jj,ii,WP,WN) => {
  ...
  if (blocks[iT * curLevel.map[0].length + jT].id == WP) { //нахождение данного
    blocks[iT * curLevel.map[0].length + jT].id = WN; //смена id
  } else if (blocks[iT * curLevel.map[0].length + jT + 1].id == WP && (WP == 'g' || WP == 's')) { //нахождение данного
    blocks[iT * curLevel.map[0].length + jT + 1].id = WN; //смена id
  } else if (blocks[iT * curLevel.map[0].length + jT - 1].id == WP && (WP == 'g' || WP == 's')) { //нахождение данного
    blocks[iT * curLevel.map[0].length + jT - 1].id = WN; //смена id
  }
}
