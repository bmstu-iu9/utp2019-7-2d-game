    ...
    for (let i = 0 ; i < map.length ; i++) {
        for (let j = 0 ; j < map[i].length ; j++) {
            if (map[i][j] == ' ') {
              ...
              var bot1 = new BotClass(1,j * 32 ,i * 32 - 64, 3.5); // создаем с данными координатами и типом
              bots.push(bot1); // вносим в массив ботов
              characters.push(bot1);
              var o = new MapObject(' ', i, j);
            } else if (map[i][j] == '2') {
              var bot2 = new BotClass(2,j * 32 ,i * 32 - 64, 1); // создаем с данными координатами и типом
              bots.push(bot2); // вносим в массив ботов
              characters.push(bot2);
              var o = new MapObject(' ', i, j);
            } else if (map[i][j] == '3') {
              var bot3 = new BotClass(3,j * 32 ,i * 32 - 64, 2); // создаем с данными координатами и типом
